...
1) Edit the [`070-ValidatingWebhookConfiguration.yaml`](070-ValidatingWebhookConfiguration.yaml) file and update the `caBundle` field to contain the Base64 encoded public key of your CA.
   You can get the Base64 encoded public key for example using the `base64` utility:
   ```
...
2) Create the `strimzi-drain-cleaner` namespace:
   ...
   ```
3) Create the Secret named `strimzi-drain-cleaner` with the `tls.crt` and `tls.key` files.
   ...
   ```
4) With all the files prepared now, you can deploy Strimzi Drain Cleaner:
   ```
   kubectl apply -f ./
   ```
